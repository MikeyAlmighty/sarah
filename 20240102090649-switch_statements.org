...
- The ~value1~, ~...~, and ~valueN~ must have the *same* data type as the value of the =switch-expression=.
    Note that ~value1~, ~...~, and ~valueN~ are *constant* *expressions*, they cannot contain variables like ~1 + x~.
...
- The keyword *break* is *optional*. (*break* *statement* immediately *ends* the *switch* *statement*).
...
You can’t have a *case* statement of type ~String~ if the *switch* statement variable is of type ~int~, since the =types= are /incomparable/.
