...
- ~Protected access~ allows everything that [[id:0d8e8caf-96d3-4fd2-b0ae-460b7b93d6ae][package access]] does, and more.
- ~protected access~ modifier adds the ability to access members of a =parent class=.
...
Running this program prints floating twice:
- Once from calling ~floatInWater()~
- Once from the print statement in ~swim()~.
...
- A ~member~ is used /through/ a *variable*.
    ...
    If it is the ~parentClass~ it does *not* allow access.)
