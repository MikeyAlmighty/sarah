...
- ~BadCygnet~ is in the ~pond.swan~ =package=, and the code *doesn’t compile*.

** Note

- When there is *no* access modifier on a member, only =classes= in the *same package* can *access* the member.
