...
In the example above, if ~duck~ *is* =null=, the conditional prevents a =NullPointerException= from being thrown, since the evaluation of ~duck.getAge() < 5~ is never reached.
...
Because ~rabbit >= 6~ is *true*, the increment operator on the =RHS= of the expression is *never* evaluated, so the output is =6=.
