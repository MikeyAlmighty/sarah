...
In the implementation, changing *just* the ~return type~ (as shown at =(1)= and =(2)= in the preceding example) is *not enough* to *overload* a =method= and will be flagged as a *compile-time* *error*.
...
- The following =method= *overloads* are *valid*.
...
- When the *primitive* ~int~ version *is* provided, there is *no* reason for Java to do the extra work of [[id:2af44b3c-ac25-4fc8-a372-20adc491f7f5][auto boxing]].
