...
The parameters ~sourcePosition~ and ~targetPosition~ indicate the *starting* positions in =sourceArray= and =targetArray=.
...
It is a simple and direct way to duplicate an array in Java.

The ~clone()~ method creates a *new* =array= of the *same* *type* and *copies* *all* =elements=.

It works well for primitive arrays (like ~int[]~, ~double[]~), where each element is copied separately.

We must note though that, for *arrays* of =objects=, ~clone()~ performs what we call *shallow* copy, meaning that only =references= are *copied*, not the actual =objects=.
