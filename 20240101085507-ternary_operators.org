...
- [[id:a29b45cb-d2f6-49fd-90c7-4c8f1b8e898a][Conditional Operator]]
...
For the exam, you should know that there is *no* requirement that ~second~ and ~third~ expressions in ternary operations have the *same* data types, although it does come into play when combined with the [[id:32c06110-1f45-4d85-9583-74d13aaa083f][assignment operator]]:
...
~System.out.print()~ does not care that the expressions are *different* types, because it can convert both to ~Object~ values and call ~toString()~ on them.
...
The following code assigns =1=, =0=, or =−1= to status if ~n1 > n1~, ~n1 ~==~ ~n2~, or ~n1 < n2~:
