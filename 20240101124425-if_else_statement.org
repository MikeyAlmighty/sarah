...
Another good reason for doing this is to avoid errors that are difficult to detect.
...
This statement does *not* have *compile* errors.
- It assigns =true= to ~~even~, so ~even~ is always =true=.
